var Pg = require('pg')
// const DATABASE = require.main.require('./config.json').dbUrl
const DATABASE = 'postgres://sg:sguard@localhost/sguard'
const local = 'postgres://sg:sguard@localhost/sguard'

class Repo {

    static Delete(values, columns, table, callback) {
        // TODO: Make SQL clauses
    }

    static Insert(values, columns, table, callback) {
        Pg.connect(DATABASE, (error, client, done) => {
            if (error) {
                console.log(error)
                return callback(error.name)
            }

            let valueStr = Array.from(new Array(columns.length),(v, i) => i+1).map((i) => `$${i}`).join(',')
            let query = `INSERT INTO ${table} (${columns.join(',')}) VALUES(${valueStr})`

            client.query(query, values, (err, result) => {
                done()

                if (err) {
                    console.log(err)
                    return callback(err)
                } else {
                    return callback(null, true)
                }
            })
        })
    }

    static Select(columns, table, wheres, values, callback) {
        Pg.connect(DATABASE, (error, client, done) => {
            if (error) {
                console.log(error)
                return callback(error.name)
            }

            let select = columns && columns.length > 0 ?
                columns.join(', ') : '*'

            let query = `SELECT ${select} FROM ${table}`
            let where = this.makeWhereClause(wheres, values)

            if (where) {
                client.query(query + where, values, (err, result) => {
                    done()

                    if (err) {
                        return callback(err)
                    } else {
                        return callback(null, result.rows)
                    }
                })
            } else {
                client.query(query, (err, result) => {
                    done()

                    if (err) {
                        console.log(err)
                        return callback(err)
                    } else {
                        return callback(null, result.rows)
                    }
                })
            }
        })
    }

    static SelectAll(table, callback) {
        Pg.connect(DATABASE, (error, client, done) => {
            if (error) {
                console.log(error)
                return callback(error.name)
            }

            let query = 'SELECT * FROM members';
            client.query(query, (err, result) => {
                done()

                if (err) {
                    return callback(err)
                } else {
                    return callback(null, result.rows)
                }
            })
        })
    }

    static makeWhereClause(columns, values) {
        if (columns && values && columns.length > 0 && values.length > 0) {
            if (columns.length === values.length) {
                let wheres = []
                for (let i = 0; i < columns.length; i++) {
                    wheres.push(`UPPER(${columns[i]})=UPPER($${i+1})`)
                }
                return ` WHERE ${wheres.join(' AND ')}`
            } else {
                throw new Error('Where clause input mismatch.')
            }
        } else {
            return ''
        }
    }
}

module.exports = Repo